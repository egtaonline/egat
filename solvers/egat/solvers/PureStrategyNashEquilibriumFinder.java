...
public class PureStrategyNashEquilibriumFinder {
	public static List<Outcome> findAllPSNE(StrategicGame game){
		List<Outcome> outcomes = new ArrayList<Outcome>();
			for(Outcome outcome : (game.getClass().isInstance(SymmetricGame.class) ? Games.symmetricTraversal((SymmetricMultiAgentSystem) game) : Games.traversal(game)))
	...
	public static boolean isPSNE(Outcome outcome, StrategicGame game) {
		return Games.regret(outcome, game) == 0;
	}
}
